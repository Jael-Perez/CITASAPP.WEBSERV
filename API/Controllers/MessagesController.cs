...
public class MessagesController : BaseApiController
{
    ...
    [HttpPost]
    public async Task<ActionResult<MessageDto>> CreateMessage(CreateMessageDto cm)
    {
        var username = User.GetUsername();

        if (username == cm.RecipientUsername.ToLowerInvariant())
        {
            return BadRequest("No te puedes enviar mensajes a ti mismo");
        }

        var sender = await _userRepository.GetUserByUsernameAsync(username);
        var recipient = await _userRepository.GetUserByUsernameAsync(cm.RecipientUsername);
    ...
    {
        mp.Username = User.GetUsername();
        var messages = await _messageRepository.GetMessagesForUserAsync(mp);
